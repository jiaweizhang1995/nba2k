...
1. **`draft_pick` 在 AGENT 模式下语义含糊**——我的每次调用都只推进到「等待我方选择」，随后选秀却自动完成（含我方签位被 AI 代选）。应改为：轮到我方时必须显式 `draft_pick {prospectId}` 才生效，否则停在等待态
...
4. **49胜→26胜的无伤病崩盘**值得查模拟方差——同核心健康打满，化学反应 77，战绩腰斩 23 场，波动幅度偏大
